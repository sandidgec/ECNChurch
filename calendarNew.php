<!DOCTYPE html>
<html>
<head>
    <?php require_once("lib/head-utils.php"); ?>
    <?php require_once("indexNavBar.php");?>
    <!-- Custom styles for this template -->
    <link href="/lib/css/stylecalendar.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Pontano+Sans" rel="stylesheet">
    <title>Calendar</title>

</head>
<body>

<div id="bg">
    <div id="search-container">
        <div id="search-bg"></div>
        <div id="search">

            <div id="bg">
                <div id="search-container">
                    <div id="search-bg"></div>
                    <div id="search">
                        
                        <div id="buttonTop" class="row">
                            <div class="col-xs-3">
                                <a href="sanJuanEvents.php" id="sjrb" class="btn btn-default btn-lg btn-block active" role="button">San Juan Region Events</a>
                            </div>
                            <div class="col-xs-3">
                                <a href="southEastEvents.php" id="serb" class="btn btn-default btn-lg btn-block active" role="button">South E. Region Events</a>
                            </div>
                            <div class="col-xs-3">
                                <a href="utahEvents.php" id="urb" class="btn btn-default btn-lg btn-block active" role="button">Utah Region Events</a>
                            </div>
                            <div class="col-xs-3">
                                <a href="churchService.php" id="ewe" class="btn btn-default btn-lg btn-block active" role="button">ECN Wide Event</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>



  <div id="calendar-wrap"><!--start month------------------------------------------------------------------------------------------>

                <h1>December 2016</h1>

                <div id="calendar">
                    <ul class="weekdays">
                        <li>Sunday</li>
                        <li>Monday</li>
                        <li>Tuesday</li>
                        <li>Wednesday</li>
                        <li>Thursday</li>
                        <li>Friday</li>
                        <li>Saturday</li>
                    </ul>
                    <ul class="days">
                        <li class="day other-month">
                            <div class="date">27</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">28</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                         <li class="day other-month">
                            <div class="date">29</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                         <li class="day other-month">
                            <div class="date">30</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">1</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">2</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                   BLM San Juan County UT Forum
                                    @ St. Christopher's TBA
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">3</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">4</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">5</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">6</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    Morning Prayer<br><!--SER---------------->
                                    Bible Study @5:30 pm
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                            <div class="event">
                                <div class="event-desc">
                                    EDC Meeting at the
                                    Bosque Ctr. In ABQ
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">7</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                    Craft Class @7pm
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">8</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">9</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">10</div>
                            <div class="event">
                                <div class="event-desc">
                                    ECN Meeting @ All Saints
                                    10am-3pm
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">11</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    SJR Combined Sunday Service @ All Saints<!--SJR---------------->
                                    Bishop Dave Presiding
                                </div>
                            </div>
                            <div class="event-ser">
                                <div class="event-desc">
                                   St. Marks Holiday Dinner @1pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">12</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">13</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">14</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">15</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">16</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">17</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">18</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">19</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">20</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">21</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">22</div>
                        </li>
                        <li class="day">
                            <div class="date">23</div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Pagent Practice<!--SJR---------------->
                                    (Pizza to Share)
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">24</div>
                            <div class="event">
                                <div class="event-desc">
                                   Christmas Eve
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Family Eucharist 6pm<!--SJR---------------->
                                </div>
                            </div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        10pm Christmas Eve Service<!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">25</div>
                            <div class="event">
                                <div class="event-desc">
                                    Christmas Day
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">26</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">27</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">28</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">29</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">30</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">31</div>
                        </li>
                    </ul>
                </div><!--/. calendar -->
            </div> <!--/. wrap -->

<?php /*


            <div id="calendar-wrap"><!--start month------------------------------------------------------------------------------------------>

                <h1>November 2016</h1>

                <div id="calendar">
                    <ul class="weekdays">
                        <li>Sunday</li>
                        <li>Monday</li>
                        <li>Tuesday</li>
                        <li>Wednesday</li>
                        <li>Thursday</li>
                        <li>Friday</li>
                        <li>Saturday</li>
                    </ul>
                    <ul class="days">
                        <li class="day other-month">
                            <div class="date">30</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">31</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">1</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">2</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">3</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">4</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">5</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">6</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">7</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">8</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">9</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">10</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                   Utah Regional Council MTG
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">11</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    St. Marks Cake Walk @6pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">12</div>
                            <div class="event">
                                <div class="event-desc">
                                    ECN Meeting @ GSM
                                    10am-3pm
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">13</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">14</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">15</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">16</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">17</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">18</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">19</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">20</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                    Thanksgiving Dinner @ St. John's<br><!--UTR---------------->
                            </div>
                            <div class="event-ser">
                                <div class="event-desc">
                                        Thanksgiving Dinner @ GSM<br><!--SER---------------->
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Combined Thanksgiving Dinner @ All Saints<!--SJR---------------->
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">21</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">22</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">23</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">24</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                    Happy Thanksgiving
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">25</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    St. Marks Cake Walk @6pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">26</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">27</div>
                        </li>
                        <li class="day">
                            <div class="date">28</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">29</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">30</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">1</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">2</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">3</div>
                        </li>
                    </ul>
                </div><!--/. calendar -->
            </div> <!--/. wrap -->

            <div id="calendar-wrap"><!--start month------------------------------------------------------------------------------------------>

                <h1>October 2016</h1>

                <div id="calendar">
                    <ul class="weekdays">
                        <li>Sunday</li>
                        <li>Monday</li>
                        <li>Tuesday</li>
                        <li>Wednesday</li>
                        <li>Thursday</li>
                        <li>Friday</li>
                        <li>Saturday</li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">2</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">3</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">4</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">5</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">6</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">7</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">8</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">9</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">10</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">11</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">12</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">13</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">14</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">15</div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    Corn Harvesting  8:00 am @ HomerDale CommunityFarm
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">16</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">17</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">18</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">19</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">20</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">21</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event">
                                <div class="event-desc">
                                    Diocese Arizona Convention
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">22</div>
                            <div class="event">
                                <div class="event-desc">
                                    Diocese Arizona Convention
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">23</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                            <div class="event">
                                <div class="event-desc">
                                    Diocese Arizona Convention
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">24</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">25</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">26</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Navajo Hymn Class<!--SJR---------------->
                                    6pm Pot Luck Style Dinner
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">27</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event">
                                <div class="event-desc">
                                    Diocese of Rio Grande Convention
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">28</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Fall Festival @6pm All Saints <!--SJR---------------->
                                   Food, games, cake walk and more!
                                </div>
                            </div>
                            <div class="event">
                                <div class="event-desc">
                                    Diocese of Rio Grande Convention
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">29</div>
                            <div class="event">
                                <div class="event-desc">
                                    Diocese of Rio Grande Convention
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    Halloween Party @ StChristopher  5 -8 pm<br>
                                    Costume Contest,Food, Cake WalkLots of Fun!
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">30</div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Joint Service St. Michaels @ 10am <!--SJR---------------->
                                    Pot Luck Style Lunch
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">31</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">1</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">2</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">3</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">4</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">5</div>
                        </li>
                    </ul>
                </div><!--/. calendar -->
            </div> <!--/. wrap -->



            <div id="calendar-wrap"><!--start month------------------------------------------------------------------------------------------>

                <h1>September 2016</h1>

                <div id="calendar">
                    <ul class="weekdays">
                        <li>Sunday</li>
                        <li>Monday</li>
                        <li>Tuesday</li>
                        <li>Wednesday</li>
                        <li>Thursday</li>
                        <li>Friday</li>
                        <li>Saturday</li>
                    </ul>
                    <ul class="days">
                        <li class="day other-month">
                            <div class="date">28</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">29</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        Navajo Nation Food Distribution<br><!--UTR---------------->
                                        10am
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">30</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">31</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">1</div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    Bishop Baileys Surgery<!--SJR---------------->
                                    Please keep him in prayer
                                </div>
                            </div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        6PM Utah Region Council Meeting @ St. Christophers<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">2</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Rev. Cathlena Away until the 20th<!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">3</div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                       
                                    </a>
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">4</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">5</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">6</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">7</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        No Hymnal Study<!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">8</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">9</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">10</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">11</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">12</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">13</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">14</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        No Hymnal Study<!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">15</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">16</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">17</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">18</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">19</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">20</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">21</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Navajo Hymnal Study @6pm<!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">22</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">23</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">24</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">25</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">26</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">27</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">28</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Navajo Hymnal Study @6pm<!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">29</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Feast Day of St. Michaels of
                                        All Angels<!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">30</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">1</div>
                        </li>
                    </ul>
                </div><!--/. calendar -->
            </div> <!--/. wrap -->

            <div id="calendar-wrap"><!--start month------------------------------------------------------------------------------------------>

                    <h1>August 2016</h1>

                <div id="calendar">
                    <ul class="weekdays">
                        <li>Sunday</li>
                        <li>Monday</li>
                        <li>Tuesday</li>
                        <li>Wednesday</li>
                        <li>Thursday</li>
                        <li>Friday</li>
                        <li>Saturday</li>
                    </ul>
                    <ul class="days">
                        <li class="day other-month">
                            <div class="date">31</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">1</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                            <div class="event-utah">
                            <div class="event-desc">
                                <a href="utahRegion.php">
                                    Navajo Nation Food Distribution<br><!--UTR---------------->
                                    10am
                                </a>
                            </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">2</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                            <div class="event-desc">
                                <a href="utahRegion.php">
                                    9AM Staff Meeting<br><!--UTR---------------->
                                    <br>
                                    10AM Bible Study @ St. Johns
                                </a>
                            </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">3</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        Bible Study<!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">4</div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        Organize items for SALE<br><!--SJR---------------->
                                        8am Old Hospital
                                    </a>
                                </div>
                            </div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        6PM Utah Region Council Meeting @ St. Christophers<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">5</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">6</div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        Flea Market SALE<br><!--SJR---------------->
                                        8am-2pm
                                    </a>
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">7</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">8</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">9</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">10</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">11</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">12</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">13</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">14</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">15</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">16</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">17</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="">
                                        Coder Bootcamp Info/Sigh Up<br>
                                    11am-1pm call for info
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">18</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">19</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">20</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        ECN Council MTG<br>
                                        Bluff, UT 10am-3pm
                                    </a>
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">21</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">22</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">23</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">24</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>

                        </li>
                        <li class="day">
                            <div class="date">25</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">26</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">27</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">28</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">29</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">30</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        9AM Staff Meeting<br><!--UTR---------------->
                                        <br>
                                        10AM Bible Study @ St. Johns
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">31</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="utahRegion.php">
                                        10:30 to 1PM Food Bank & Closet @ St. Christopher's<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">1</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">2</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">3</div>
                        </li>
                    </ul>
                </div><!--/. calendar -->
            </div> <!--/. wrap -->



<div id="calendar-wrap"><!--start month------------------------------------------------------------------------------------------>
                <header>
                    <h1>July 2016</h1>
                </header>
                <div id="calendar">
                    <ul class="weekdays">
                        <li>Sunday</li>
                        <li>Monday</li>
                        <li>Tuesday</li>
                        <li>Wednesday</li>
                        <li>Thursday</li>
                        <li>Friday</li>
                        <li>Saturday</li>
                    </ul>
                    <ul class="days">
                        <li class="day other-month">
                            <div class="date">26</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">27</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">28</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>

                        </li>
                        <li class="day other-month">
                            <div class="date">29</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Food Bank 10-1<!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Bible Study<br><!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">30</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">1</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">2</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">3</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">4</div>
                            <div class="event">
                                <div class="event-desc">
                                    Holiday<br>
                                    4th of July<br>
                                    Offices Closed<br>
                                </div>
                            </div>
                        </li>

                        <li class="day">
                            <div class="date">5</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Bible Study<!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>

                        <li class="day">
                            <div class="date">6</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Bible Study<!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>

                        <li class="day">
                            <div class="date">7</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Utah Region Council Mtg<!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">8</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">9</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">10</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">11</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">12</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Bible Study<!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">13</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Food Bank 10-1<!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Bible Study<!--SJR------------------>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">14</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">15</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">16</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">17</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        S.E.R. Council Mtg. 1pm<!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        S.J.R. Council Mtg. 1pm<!--SJR------------------>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">18</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed<br>
                                </div>
                            </div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        PM V.B.S. St. Marks<!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">19</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                        PM V.B.S. @ St. Marks
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Bible Study<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">20</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                        PM V.B.S. @ St. Marks
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Bible Study<br><!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">21</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                        PM V.B.S. @ St. Marks
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">22</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">23</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">24</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">25</div>
                            <div class="event">
                                <div class="event-desc">
                                    Offices Closed<br>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">26</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Bible Study<br><!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">27</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Food Bank 10-1<!--UTR---------------->
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Bible Study<br><!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">28</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">29</div>
                            <div class="event-ser">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Morning Prayer<br><!--SER---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">30</div>
                            <div class="event-utah">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Leon's farwell Dinner<!--UTR---------------->
                                    </a>
                                </div>
                            </div>

                        </li>
                        <li class="day">
                            <div class="date">31</div>
                            <div class="event">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Sunday Services<br>
                                    </a>
                                </div>
                            </div>
                            <div class="event-sjr">
                                <div class="event-desc">
                                    <a href="churchService.php">
                                        Combined Service @ 10AM St. Luke's Rev. Inez Retirement Dinner<!--SJR---------------->
                                    </a>
                                </div>
                            </div>
                        </li>
                        <li class="day other-month">
                            <div class="date">1</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">2</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">3</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">3</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">5</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">6</div>
                    </ul>
                </div><!--/. calendar -->
            </div> <!--/. wrap -->

            <div id="calendar-wrap"><!--start month------------------------------------------------------------------------------------------>
                <header>
                    <h1>June 2016</h1>
                </header>
                <div id="calendar">
                    <ul class="weekdays">
                        <li>Sunday</li>
                        <li>Monday</li>
                        <li>Tuesday</li>
                        <li>Wednesday</li>
                        <li>Thursday</li>
                        <li>Friday</li>
                        <li>Saturday</li>
                    </ul>
                    <ul class="days">
                        <li class="day other-month">
                            <div class="date">29</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">30</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">31</div>
                        </li>
                        <li class="day">
                            <div class="date">1</div>
                        </li>
                        <li class="day">
                            <div class="date">2</div>
                        </li>
                        <li class="day">
                            <div class="date">3</div>
                        </li>
                        <li class="day">
                            <div class="date">4</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">5</div>
                        </li>
                        <li class="day">
                            <div class="date">6</div>
                        </li>
                        <li class="day">
                            <div class="date">7</div>
                        </li>
                        <li class="day">
                            <div class="date">8</div>
                        </li>
                        <li class="day">
                            <div class="date">9</div>
                        </li>
                        <li class="day">
                            <div class="date">10</div>
                            <div class="event">
                                <div class="event-desc">
                                    40th Convocation of the Episcopal Church in Navajoland-Registration
                                </div>
                                <div class="event-time">
                                    4:00pm-7:30pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">11</div>
                            <div class="event">
                                <div class="event-desc">
                                    40th Convocation of the Episcopal Church in Navajoland-Lectures
                                </div>
                                <div class="event-time">
                                    7:30pm-8:30pm
                                </div>
                            </div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">12</div>
                            <div class="event">
                                <div class="event-desc">
                                    40th Convocation of the Episcopal Church in Navajoland-Business
                                </div>
                                <div class="event-time">
                                    7:30pm-1:30pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">13</div>
                            <div class="event">
                                <div class="event-desc">
                                    All Saints VBS
                                </div>
                                <div class="event-time">
                                    9:00pm-12:00pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">14</div>
                            <div class="event">
                                <div class="event-desc">
                                    All Saints VBS
                                </div>
                                <div class="event-time">
                                    9:00pm-12:00pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">15</div>
                            <div class="event">
                                <div class="enent-desc">
                                    All Saints VBS
                                </div>
                                <div class="event-time">
                                    9:00pm-12:00pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">16</div>
                            <div class="event">
                                <div class="event-desc">
                                    All Saints VBS
                                </div>
                                <div class="event-time">
                                    9:00pm-12:00pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">17</div>
                        </li>
                        <li class="day">
                            <div class="date">18</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">19</div>
                        </li>
                        <li class="day">
                            <div class="date">20</div>
                        </li>
                        <li class="day">
                            <div class="date">21</div>
                        </li>
                        <li class="day">
                            <div class="date">22</div>
                        </li>
                        <li class="day">
                            <div class="date">23</div>
                            <div class="event">
                                <div class="event-desc">
                                    Cohort 1 Meeting with Cultivating Coders &ECN
                                </div>
                                <div class="event-time">
                                    4:30pm-6:30pm
                                </div>
                            </div>
                        </li>
                        <li class="day">
                            <div class="date">24</div>
                        </li>
                        <li class="day">
                            <div class="date">25</div>
                        </li>
                    </ul>
                    <ul class="days">
                        <li class="day">
                            <div class="date">26</div>
                        </li>
                        <li class="day">
                            <div class="date">27</div>
                        </li>
                        <li class="day">
                            <div class="date">28</div>
                        </li>
                        <li class="day">
                            <div class="date">29</div>
                        </li>
                        <li class="day">
                            <div class="date">30</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">1</div>
                        </li>
                        <li class="day other-month">
                            <div class="date">2</div>
                        </li>
                    </ul>
                </div><!--/. calendar -->
            </div> <!--/. wrap -->
*/?>
            Episcopal Church in Navajoland P.O. Box 720 Farmington, NM 87499 Phone: (505) 327-7549 — Main

        </div>
    </div>
</div>

<ol class="breadcrumb">
    <li><a href="index.php">Home</a></li>
    <li><a href="areaMissContact.php">Area Mission Contact</a></li>
    <li><a href="calendarNew.php">Calendar</a></li>
    <li><a href="findaChurch.php">Find A Church</a></li>
    <li><a href="missionsRetreats.php">Missions/Retreats</a></li>
    <li class="active"></li>
</ol>
</body>

</html>