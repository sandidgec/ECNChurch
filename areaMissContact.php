<!DOCTYPE html>
<html>
<head>
    <?php require_once("lib/head-utils.php"); ?>
    <?php require_once("indexNavBar.php");?>
    <title>Area Mission Office Contact</title>
    <!-- Custom styles for this template -->
    <link href="/lib/css/style.css" rel="stylesheet">

</head>
<body>

    <div class="logoimage">
        <img src="/lib/img/episcopalchurchinnavajolandlogo_480.png" class="img-responsive" alt="Awesome.jpg">
    </div>
    <div class="container">
        <div class="row">
            <div class="firstline">
                <a class="btn btn-primary btn-lg center-block" href="#contact" role="button">
                    <span class="text-big">Contact Us</span>
                </a>

            </div>
        </div>
    </div>

 <div class="container-all">
            <div class="historyJumbo">
                <div class="firstline">
                    <h2>Area Mission Office</h2>
                </div>
            </div>

            <div class="container">
                <div class="row">

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/dave.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rt. Rev. David E. Bailey</h3>
                                <p class="text-center">
                                    Bishop
                                </p>
                                <p class="text-center">
                                    <a href="mailto:ecndbailey@gmail.com">ecndbailey@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>


                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/chan.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Chan de Anaya</h3>
                                <p class="text-center">
                                    Canon Missioner for ECN
                                </p>
                                <p class="text-center">
                                    <a href="mailto:ecncanaya@gmail.com">ecncanaya@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/cat.jpg" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Cathlena Plummer</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/halana.jpg" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Helena Begay</h3>
                                <p class="text-center">
                                    Bookkeeper
                                </p>
                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                </div>

      <div class="container-all">


                    <div class="historyJumbo">
                        <div class="firstline">
                            <h2>Clergy</h2>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/chan.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Chan Anaya</h3>

                                <p class="text-center">
                                    Canon Missioner for ECN
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/dave.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rt. Rev. David E. Bailey</h3>

                                <p class="text-center">
                                    Bishop of The Episcopal Church in Navajoland
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/pat.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Patricia Cook</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/cornelia.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Cornelia Eaton</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/lily.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Lay Pastor Lillie Henderson</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/paulaH.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Deacon Paula Henson</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/cynthia.jpg" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Cynthia Hizer</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/catsmom.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Catherine Plummer</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/cat.jpg" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Cathlena Plummer</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/LeonS.JPG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Deacon Leon Sampson</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>


                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/carol.png" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Carol Tookey</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="row">
                            <img src="/lib/img/inez.PNG" alt="Person"
                                 class="img-responsive img-circle pic center-block"/>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <h3 class="text-center">Rev. Inez Velarde</h3>

                                <p class="text-center">
                                    Priest in charge of All Saints
                                </p>

                                <p class="text-center">
                                    <a href="mailto:ecncplummer@gmail.com">ecncplummer@gmail.com</a>
                                </p>
                            </div>
                        </div>
                    </div>






                </div>
            </div>

    </div>



<!-- Contact -->
<section id="contact" class="four">
    <div class="contact1">
        <div class="container">

        <header>
            <h1>Have any questions?</h1>
        </header>

        <form method="post" action="contact.php" class="col-md-6 col-md-offset-3" id="contactForm" name="contactForm">
            <div class="form-group">
                <label for="email">Email address</label>
                <input type="email" class="form-control" id="email" name="email" placeholder="Email">
            </div>
            <div class="form-group">
                <label for="message">Message</label>
                <textarea class="form-control" id="message" name="message" rows="3" placeholder="Message"></textarea>
            </div>
            <button class="btn btn-success" id="submit" name="submit" type="submit">Send</button>
            <div class="form-group">
            </div>
        </form>
        </div>

    </div>
</section>
    <div class="addressinfo">
    <p>Episcopal Church in Navajoland P.O. Box 720 Farmington, NM 87499 Phone: (505) 327-7549 — Main</p>
    </div>
</body>

</html>